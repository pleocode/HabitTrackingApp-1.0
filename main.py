...
def cli():
    ...
    while not stop:
        ...
        if choice == "Create":
            ...
            name = questionary.select("Do you want to analyse by habit, periodicity or check the longest streak?", choices=["Habit", "Periodicity", "Longest Streak"]).ask()
            if name == "Longest Streak": # In case of selecting longest streak
                ...
                except (IndexError): # Handling error when the db is empty or there is no streak yet
                    print("The db is empty or there is no streak yet")

            else:  # In case of selecting habit or periodicity

                if name == "Habit": # In case of selecting directly from a habit

                    try:  # In case of trying to analyse on habit with an empty db or the habit has been incremented 0 times
                        name = questionary.select("What habit do you want to analyse?", choices=get_countername_list(db)).ask()
                    ...
                    except (ValueError, UnboundLocalError):  # Handling error when user tries to analyse on a habit with an empty db or the habit has been incremented 0 times
                        print("There isn't any habit created yet to analyse or this habit has been incremented 0 times")

                elif name == "Periodicity": # in case of selecting from the periodicity of a habit the habit has been incremented 0 times
                    name = questionary.select("Which periodicity do you want to analyse from?", choices=["Daily", "Weekly"]).ask()

                    try: # In case the user selects a periodicity where there is still no habit created or

                        if name == "Daily": # For daily periodicity
                            name = questionary.select("What habit do you want to analyse?", choices=get_counternameper_list(db, "Daily")).ask()
                            count = calculate_count(db, name)

                        else: # For weekly periodicity
                            name = questionary.select("What habit do you want to analyse?", choices=get_counternameper_list(db, "Weekly")).ask()
                            count = calculate_count(db, name)
